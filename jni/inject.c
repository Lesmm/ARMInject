...
{    
    ...
    DEBUG_PRINT("[+] libc_path: %s， linker_path：　%s", libc_path, linker_path); 
...
int main(int argc, char** argv) {
    ...
    char* processName = "com.android.phone";
    if (argc >= 2) {
        processName = argv[1];
    }

    pid_t target_pid;
    target_pid = find_pid_of(processName); 
    //target_pid = find_pid_of("/system/bin/servicemanager");  

    if (-1 == target_pid) {
        printf("Can't find the process %s's pid\n", processName);
        return -1;
    } else {
        printf("Find the process %s's pid is: %d\n", processName, target_pid);
    }

    if (argc >= 3) {
        char* injectSoPath = argv[2];
        inject_remote_process(target_pid, injectSoPath, "hook_entry",  "I'm parameter!", strlen("I'm parameter!"));
        printf("Injected %s to pid %d success!\n", injectSoPath, target_pid);
    } else {
        printf("No inject libxxx.so specified!\n");
    }
    return 0;
}  